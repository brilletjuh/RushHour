...
namespace RushHourSolver
{
	static class Program
	{
		...
		static bool solved = false; //Whether the solution is already found
		...
		private static IEnumerable<Tuple<byte[], Solution>> Sucessors(Tuple<byte[], Solution> state)
		{
			...
			for (byte v = 0; v < vehicleNames.Length; v++)
			{
				...
				for (byte i = 1; i <= size - (currentState[v] + vehicleLengths[v]); i++)
				{
					...
					bool good = true;
					foreach (int j in sameDir)
						if (j != v && currentState[j] == currentState[v] + i + vehicleLengths[v] - 1)
							good = false;

					// Similarly, skip if we bump in to a vehicle in the different direction
					foreach (int j in diffDir[currentState[v] + vehicleLengths[v] + i - 1])
						if (currentState[j] <= vehicleLocs[v] && currentState[j] + vehicleLengths[j] - 1 >= vehicleLocs[v])
							good = false;
				...
				for (byte i = 1; i <= currentState[v]; i++)
				{
					...
					bool good = true;
					foreach (int j in sameDir)
						if (j != v && currentState[j] + vehicleLengths[j] - 1 == currentState[v] - i)
							good = false;

					// Similarly, skip if we bump in to a vehicle in the different direction
					foreach (int j in diffDir[currentState[v] - i])
						if (currentState[j] <= vehicleLocs[v] && currentState[j] + vehicleLengths[j] - 1 >= vehicleLocs[v])
							good = false;
